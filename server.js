...
//   completedMatches / totalMatches  (0 if no matches yet)
// ─────────────────────────────────────────
...
async function distributeReward(t, match, matchRef, confirmedWinner) {
  ...
  if (confirmedWinner === "draw") {
    const aData = { ...playerA_Doc.data(), completedMatches: inc(playerA_Doc.data().completedMatches) };
    const bData = { ...playerB_Doc.data(), completedMatches: inc(playerB_Doc.data().completedMatches) };
    ...
    applyTrustUpdate(t, playerA_Ref, aData);
    applyTrustUpdate(t, playerB_Ref, bData);
    ...
    applyTrustUpdate(t, winnerRef, winnerUpdated);
    applyTrustUpdate(t, loserRef,  loserUpdated);
...
// REFERRAL SYSTEM
//
// ── Reduced rewards to control coin inflation ──
//   New user bonus:  5 coins  (was 10)
//   Referrer bonus:  5 coins  (was 15)
// ═══════════════════════════════════════════════════════════════
app.post("/apply-referral", verifyToken, async (req, res) => {
  ...
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      // Reduced referral rewards
      bonusCoins = 5; // new user gets 5 coins (was 10)
      ...
      t.update(referrerRef, {
        coins:         inc(referrerDocT.data().coins, 5), // referrer gets 5 coins (was 15)
...
// DAILY REWARD
//
// Reduced cycle to control coin inflation:
//   Day 1 → 1 coin   (was 3)
//   Day 2 → 1 coin   (was 3)
//   Day 3 → 2 coins  (was 4)
//   Day 4 → 2 coins  (was 4)
